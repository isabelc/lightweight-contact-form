...
function lcf_input_filter() {
	...
	if ( 2 != $lcf_strings['value_response'] ) {
		$pass = false;
		$fail = 'wrong';
		$lcf_strings['response_error'] = true;
	}
	...
	submitButton.onclick = function() {
		...
		if (!mathBlank) { // if math response is entered, validate it
			...
			if ( isNaN( parseInt( mathNode.value ) ) ) { // Not numeric, so add an error
				...
				lcfErrorLabel(mathNode, 'lcf_response', "<?php _e( 'Please solve the math problem. The answer must be a number.', 'lightweight-contact-form' ); ?>");
